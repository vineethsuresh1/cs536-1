...
//        ReturnStmtNode,  DotAccessNode,   AssignNode,  CallExpNode,
...
abstract class ASTnode { 
  ...
  // used to determine if there is a main function or not
  public static boolean foundMain = false;
  // Registers class for saving/using registers
  public static Registers regPool = null;
  // 
  public static int lastSeenParamsCount = 0;
  public static int currentLexicalLevel = 0;
  public static String lastSeenFunctionExit;
  ...
  public void nameAnalysis(SymTable symTab, SymTable globalTab) {
    ...
    for(int i = 0; i < myDecls.size(); i++) {
        try {
            ...
            // just means we don't need to calculate offset as it's a function
        }
    }
  }    
...
class VarDeclNode extends DeclNode {
    ...
    if (!badDecl) {  // insert into symbol table
      try {
        if (myType instanceof StructNode) {
          ...
          sym = new Sym(myType.type(), symTab.size() - 1);
        }
  ...
  public void codeGen() {
    if(currentLexicalLevel == 0) {
      if(DEBUG) System.out.println("Variable declaration of type:" + 
                                  myType.toString() + " id:" + myId.toString());
...
class FnDeclNode extends DeclNode {
  ...
  public Sym nameAnalysis(SymTable symTab) {
    ...
    lastSeenParamsCount = myFormalsList.size();
    ...
    if (symTab.lookupLocal(name) != null) {
      ...
      try {
        sym = new FnSym(myType.type(), 
                            myFormalsList.size(), lSize, symTab.size());
        symTab.addDecl(name, sym);
        myId.link(sym);
      } catch (DuplicateSymException ex) {
        System.err.println("Unexpected DuplicateSymException " +
  ...
  public void codeGen() {
    ASTnode.regPool = new Registers();
    int parameterCount = ((FnSym)myId.sym()).getNumParams();
    int localsCount = myBody.numLocals();
    lastSeenFunctionExit = Codegen.nextLabel();

    // Start of function
    // Print .text
    Codegen.generateWithComment(".text", "");
    // Print labels
    if(myId.name().equals("main")) {
      Codegen.generateWithComment(".globl main", "");
      Codegen.genLabel("main");
      Codegen.genLabel("__start");
    }
    else Codegen.genLabel("_" + myId.name());

    // increment level of lexical scope
    currentLexicalLevel+=1;

    // push return addr
    Codegen.genPush("$ra");

    // backup old frame pointer into control link
    Codegen.genPush("$fp");

    // calculate new frame pointer
    Codegen.generate("addu", "$fp", "$sp", 8 + parameterCount * 4);

    // push space for local stuff
    Codegen.generate("sub", "$sp", "$sp", localsCount * 4);

    // back up registers and create Register Pool
    regPool.saveAllRegisters();
    ...
    // Function Exit
    Codegen.genLabel(lastSeenFunctionExit);

    // Take top of stack result and put into v0
    if(!myType.type().isVoidType()) {
      Codegen.genPop("$v0");
    }

    // Load return addr
    Codegen.generateIndexed("lw", "$ra", "$fp", -4 * parameterCount);

    // Save Control Link
    String Reg0 = regPool.nextRegister();
    Codegen.generate("move", Reg0, "$fp");

    // Restore frame pointer
    Codegen.generateIndexed("lw", "$fp", "$fp", 
        ((-4 * parameterCount) - 4));
    // Restore stack pointer
    Codegen.generate("move", "$sp", Reg0);
    regPool.releaseRegister(Reg0);

    // Restore saved regs
    regPool.restoreAllRegisters();
    ...
    else Codegen.generate("jr", "$ra");

    // Decrement current level of lexical scope
    currentLexicalLevel--;
  }
...
class FormalDeclNode extends DeclNode {
  ...
  public Sym nameAnalysis(SymTable symTab) {
    ...
    if (!badDecl) {  // insert into symbol table
      try {
        sym = new Sym(myType.type(), symTab.size());
        symTab.addDecl(name, sym);
        myId.link(sym);
      } catch (DuplicateSymException ex) {
        System.err.println("Unexpected DuplicateSymException " +
  ...
  // Need a generate node to override abstract but don't actually do anything
  public void codeGen() {
...
class StructDeclNode extends DeclNode {
  ...
  public void codeGen() {
    // don't do anything because we don't handle structs
  }
...
class PostIncStmtNode extends StmtNode {
    ...
    public void codeGen() {
      myExp.codeGen();
      // Grab register
      String Reg0 = ASTnode.regPool.nextRegister();
      // Pop value
      Codegen.genPop(Reg0);
      // Do the addition
      Codegen.generate("add", Reg0, Reg0, 1);
      // Push result
    ...
    public void unparse(PrintWriter p, int indent) {
      ...
      p.println("--;");
                ...
                public void codeGen() {
                        ...
                        String Reg0 = ASTnode.regPool.nextRegister();
                        Codegen.genPop(Reg0);
                        Codegen.generate("sub", Reg0, Reg0, 1);
                        Codegen.genPush(Reg0);
                        regPool.releaseRegister(Reg0);
                }
...
class ReadStmtNode extends StmtNode {
    ...
    public void codeGen() {
      String Reg0 = regPool.nextRegister();
      try {
        IdNode currentId = ((IdNode)myExp);
        // Get type, then read it into v0
        if(currentId.sym().getType().isIntType() || 
              currentId.sym().getType().isBoolType()) {
          Codegen.generate("li", "$v0", 5);
          Codegen.generate("syscall");
        }
        // Next, load address of variable onto stack
        currentId.generateByRef();
      } catch (ClassCastException ce) {
          // No big deal, will only be IdNode anyway
      }
      // Copy input into Reg0
      Codegen.genPop(Reg0);
  ...
  public void codeGen() {
    try {
      if(IdNode.class.isInstance(myExp)) {
        IdNode currentId = ((IdNode)myExp);
        if(currentId.sym().getType().isIntType()) {
          currentId.generateByVal();
          Codegen.generate("li", "$v0", 1);
          Codegen.genPop("$a0");
          Codegen.generate("syscall");
        }
        else if(currentId.sym().getType().isBoolType()) {
          currentId.generateByVal();
          String Reg0 = regPool.nextRegister();
          Codegen.genPop(Reg0);
        ...
        else if(currentId.sym().getType().isStringType()) {
          Codegen.generate("li", "$v0", 4);
          currentId.generateByVal();
          Codegen.genPop("$a0");
          Codegen.generate("syscall");
        }
        // or this is a function
        else if(currentId.sym().getType().isFnType()) {
          currentId.generateByVal();
          Codegen.genPop("$a0");
          Codegen.generate("li", "$v0", 4);
          Codegen.generate("syscall");
        ...
        // Should be IdNode, don't process
    }
  }
...
class IfStmtNode extends StmtNode {
    ...
    public void codeGen() {
      ...
      Codegen.generate("beqz", Reg0, falseLabel);
      regPool.releaseRegister(Reg0);
      // Next generate code for true condition
      myDeclList.codeGen();
      myStmtList.codeGen();
      // After that put the falseLabel in
      Codegen.genLabel(falseLabel);
    }
...
class WhileStmtNode extends StmtNode {
    ...
    public void codeGen() {
      // Generate repeat label for while condition
      String repeatLabel = Codegen.nextLabel();
      String Reg0 = regPool.nextRegister();
      Codegen.genLabel(repeatLabel);
      regPool.releaseRegister(Reg0);
      // Generate code for decl and stmtlist
      myDeclList.codeGen();
      myStmtList.codeGen();
      // Evaluate expr
      myExp.codeGen();
      Reg0 = regPool.nextRegister();
      Codegen.genPop(Reg0);
      // If true, branch to repeatLabel
...
class ReturnStmtNode extends StmtNode {
    ...
    public void codeGen() {
      if(myExp != null) myExp.codeGen();
      Codegen.generate("b", lastSeenFunctionExit);
    }
...
class IntLitNode extends ExpNode {
    ...
    public void codeGen() {
      String Reg0 = regPool.nextRegister();
      // Load given value into register
      Codegen.generate("li", Reg0, myIntVal);
      // Then push register onto stack
      Codegen.genPush(Reg0);
      // Release register
      regPool.releaseRegister(Reg0);
    }
...
class StringLitNode extends ExpNode {
    ...
    public void codeGen() {
      ...
      String Reg0 = regPool.nextRegister();
      Codegen.generateWithComment(".data", "");
      Codegen.generateLabeled(label, ".asciiz ", "",  myStrVal);
      // Push addr of str onto stack
      Codegen.generateWithComment(".text", "");
...
class IdNode extends ExpNode {
    ...
    public void generateByVal() {
      // If global variable
      if(currentLexicalLevel > mySym.scopeLevel) {
        String Reg0 = regPool.nextRegister();
      ...
      // if in the current scope
      String Reg0 = regPool.nextRegister();
      Codegen.generate("add", Reg0, "$fp", mySym.getBase());
      Codegen.generateIndexed("lw", Reg0, Reg0, 0);
      Codegen.genPush(Reg0);
      regPool.releaseRegister(Reg0);
    }
    ...
    public void generateForFunction() {
      // pseudo-type check for function call
      if(!(mySym instanceof FnSym)) {
        System.out.println("codeGen for Function called on non-function");
        System.exit(-1);
      }
      // Call the function; params passed earlier on
...
class DotAccessExpNode extends ExpNode {
    ...
    public void codeGen() {
      // Don't do anything; this is a dot-access
    }
...
class AssignNode extends ExpNode {
  ...
  public void codeGen() {
    ...
    try {
      // Then do lhs
      ((IdNode)myLhs).generateByRef();
    } catch (ClassCastException ce) {
        System.out.println("Class Cast exception shouldn't happen in Assign");
        System.exit(-1);
    }
      String Reg0 = regPool.nextRegister();
      String Reg1 = regPool.nextRegister();
      // Address and operand
      Codegen.genPop(Reg0); // address
      Codegen.genPop(Reg1); // operand
      // Store operand into target address
      Codegen.generateIndexed("sw", Reg1, Reg0, 0);
      Codegen.genPush(Reg1);
      // Then release the registers
      regPool.releaseRegister(Reg0);
      regPool.releaseRegister(Reg1);
...
class CallExpNode extends ExpNode {
    ...
    public Type typeCheck() {
      ...
      int numParams = fnSym.getNumParams();
      if (myExpList.size() != numParams) {
        ErrMsg.fatal(myId.lineNum(), myId.charNum(), 
    ...
    public void codeGen() {
      myExpList.codeGen();
      Codegen.genLabel("#1728#CallExpNode, " +
          "check if function is called correctly");
      myId.generateForFunction();
    }
...
class PlusNode extends ArithmeticExpNode {
    ...
    public void codeGen() {
      ...
      // Take the registers
      String Reg0 = ASTnode.regPool.nextRegister();
      String Reg1 = ASTnode.regPool.nextRegister();
      // Pop values into T0 and T1
      Codegen.genPop(Reg0);
      Codegen.genPop(Reg1);
      // Do addition
...
class MinusNode extends ArithmeticExpNode {
    ...
    public void codeGen() {
      ...
      String Reg0 = ASTnode.regPool.nextRegister();
      String Reg1 = ASTnode.regPool.nextRegister();
      // Pop values into T0, T1
      Codegen.genPop(Reg0);
      Codegen.genPop(Reg1);
      // Do subtraction
      Codegen.generate("sub", Reg0, Reg0, Reg1);
      // Push result and release registers
      Codegen.genPush(Reg0);
      regPool.releaseRegister(Reg0);
      regPool.releaseRegister(Reg1);
    }
...
class EqualsNode extends EqualityExpNode {
    ...
    public void codeGen() {
      ...
      String Reg0 = regPool.nextRegister();
      String Reg1 = regPool.nextRegister();
      String Reg2 = regPool.nextRegister();
      Codegen.genPop(Reg1);
      Codegen.genPop(Reg2);
      // XOR then NOT takes care of this operation
      Codegen.generate("xor", Reg0, Reg1, Reg2);
      Codegen.generate("not", Reg0, Reg0);
      Codegen.genPush(Reg0);
      // Release the registers (like usual)
      regPool.releaseRegister(Reg0);
      regPool.releaseRegister(Reg1);
      regPool.releaseRegister(Reg2);
    }
...
class LessNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        // Push result then release registers
        Codegen.genPush(Reg0);
        regPool.releaseRegister(Reg0);
        regPool.releaseRegister(Reg1);
        regPool.releaseRegister(Reg2);

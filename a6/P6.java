...
public class P6 {
        ...
        throws IOException // may be thrown by the scanner
    {
        ...
        try {
            ...
            System.err.println("Exception thrown while parsing: "+ex);
            ex.printStackTrace();
            System.exit(-1);
        }
        if(!ErrMsg.errors) {
        ...
        // Do Code generation
        System.out.println("Beginning Code generation stage.");
        Codegen.initialize("spim_" + args[1] + ".s");
        ((ProgramNode)root.value).codeGen();
        Codegen.writeFile();
        System.out.println("Code generation complete. Output file: " +
                               args[1] + ".s");
        return;
    }
}
